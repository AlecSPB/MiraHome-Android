...
public class SetWifiActivity extends BaseActivity {
    ...
    private View layout_wifi_related;//wifi相关
    private View layout_scan_wifi;//扫描设备
    private View layout_tip_connect;
    ...
    private TextView tv_confirm_guide;
...
//        judgeWifiConnected();
    ...
    private void judgeWifiConnected() {
        ...
        layout_tip_connect.setVisibility(View.GONE);
    }
...
//                InitApplicationHelper.sp.edit().putString(MConstants.DEVICE_ID, deviceId).commit();
                ...
                //添加成功之后,跳转到已有设备列表界面
                startActivity(new Intent(context, ExistingDeviceActivity.class));
                context.finish();
            } else {
                //添加失败直接跳转错误界面
